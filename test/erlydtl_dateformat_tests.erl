...
all_dateformats_test_() ->
    [{Title, [test_fun(Param, Test) || Test <- Tests]}
     || {Title, Param, Tests} <- test_defs()].

test_fun(Param, {Input, Expect}) ->
    ?_assertMatch(Expect, erlydtl_dateformat:format(Param, Input)).
